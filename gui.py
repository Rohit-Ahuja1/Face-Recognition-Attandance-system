...
class App(customtkinter.CTk):
    ...
    def delete_student(self, roll_number):
        """Takes roll_number and deletes student's photo and information from the database"""
        ...
        if roll_number in self.student_data['roll_num'].values:
            ...
            self.dname = self.student_data[self.student_data["roll_num"] == roll_number]['name'].iloc[0]
            self.droll = self.student_data[self.student_data["roll_num"] == roll_number]['roll_num'].iloc[0]
            self.ddept = self.student_data[self.student_data["roll_num"] == roll_number]['department'].iloc[0]

            # Deleting the student from the database and images
            self.student_data = self.student_data[self.student_data["roll_num"] != roll_number]

            # Delete the student's photo from the images folder
            photo_path = f"images/{roll_number}.jpg"
            ...
            # Display the deleted student's details
            self.deleted_student_display = customtkinter.CTkLabel(master=self.delete_student_frame, text=f"Deleted Student:\n\n {self.dname} \n {self.droll} \n {self.ddept}")
            self.deleted_student_display.grid(row=5, column=1, padx=(10, 10), pady=(10, 10), sticky="ew")
        else:
            # Display message if no student with the given roll number was found
            self.deleted_student_display = customtkinter.CTkLabel(master=self.delete_student_frame, text=f"No student with roll number {roll_number}")
            self.deleted_student_display.grid(row=5, column=1, padx=(10, 10), pady=(10, 10), sticky="ew")
    ...
    def train_button_event(self):
        ...
        try:
            result = subprocess.run(["python", "encoder.py"], check=True, text=True, capture_output=True)
            print(result.stdout)  # print the output of encoder.py
        except subprocess.CalledProcessError as e:
            print(f"Error running encoder.py: {e.stderr}")

    def detect_button_event(self):
        """runs the main.py to mark attendance"""
        ...
        try:
            ...
            print(f"Error running main.py: {e.stderr}")
